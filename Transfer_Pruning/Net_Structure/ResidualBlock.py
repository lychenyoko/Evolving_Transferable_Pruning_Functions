...
class ResidualBlock:
    def __init__(self, inp_chan, out_chan, downsample = False, BN_Var_Epsilon = 1e-5, BN_Momentum = 0.99, is_first_block = False, param_list = None):
        ...
        if param_list == None:
            self.W_conv1 = weight_variable([CONV_KER_SIZE, CONV_KER_SIZE, inp_chan, out_chan], 'W_conv1')
            self.W_conv2 = weight_variable([CONV_KER_SIZE, CONV_KER_SIZE, out_chan, out_chan], 'W_conv2')
            if is_first_block:
                self.batch_norm2 = BatchNormLayerForCNN(Var_Epsilon = BN_Var_Epsilon, Momentum = BN_Momentum, num_channel = inp_chan)
                self.bn_list = [self.batch_norm2]
            else:
                self.batch_norm1 = BatchNormLayerForCNN(Var_Epsilon = BN_Var_Epsilon, Momentum = BN_Momentum, num_channel = inp_chan)
                self.batch_norm2 = BatchNormLayerForCNN(Var_Epsilon = BN_Var_Epsilon, Momentum = BN_Momentum, num_channel = out_chan)
                self.bn_list = [self.batch_norm1,self.batch_norm2]
            self.inp_sel = [True] * inp_chan

        else:
            weight_param, bn_param, inp_sel = param_list[:2], param_list[2:-1], param_list[-1]
        ...
        self.weight_list = [self.W_conv1,self.W_conv2]
    ...
    def inference(self, input_tensor, is_training = False, conv_mask_list = None):        
        ...
        if conv_mask_list == None:
            conv1_mask,conv2_mask = [[True] * W.shape[-2] for W in self.weight_list]

        else:
            conv1_mask,conv2_mask = conv_mask_list
        ...
        with tf.variable_scope('conv1_in_block'):
            ...
            if self.is_first_block:
                ...
                conv1 = tf.nn.conv2d(act1_post_mask, self.W_conv1, strides=[1, stride, stride, 1], padding='SAME')
                
        with tf.variable_scope('conv2_in_block'):
            bn2 = self.batch_norm2.feed_forward(conv1,is_training)
            relu2 = tf.nn.relu(bn2)
            relu2_post_mask = tf.multiply(relu2,conv2_mask)
            conv2 = tf.nn.conv2d(relu2_post_mask, self.W_conv2, strides=[1, 1, 1, 1], padding='SAME')

        layer_dict = {'Relu1 Output': act1_post_mask, 'Relu2 Output': relu2_post_mask}
        ...
        with tf.variable_scope('shortcut_connection'):
            ...
            output_tensor = conv2 + padded_input
    ...
    def block_save(self,m_sess):
        ...
        block_param = weight_param + bn_param + [self.inp_sel]
        return block_param
...
### ============================================== Method Defined for Pruning ==============================================
    ...
    def block_compression(self, mask_list, m_sess):
        conv1_mask,conv2_mask = mask_list
        bl_param = self.block_save(m_sess)
        weight_param, bn_param, inp_sel = bl_param[:len(self.weight_list)], bl_param[len(self.weight_list):-1], bl_param[-1]
        ...
        comp_W2 = W2[:,:,conv2_mask,:]
        comp_weight_param = [comp_W1,comp_W2]

        # Compressed the BN param
        if self.is_first_block:
            bn2 = bn_param[0]
            comp_bn2 = [bn_p[conv2_mask] for bn_p in bn2]
            comp_bn_param = [comp_bn2]
        else:
            bn1,bn2 = bn_param
            comp_bn1 = [bn_p[conv1_mask] for bn_p in bn1]
            comp_bn2 = [bn_p[conv2_mask] for bn_p in bn2]
            comp_bn_param = [comp_bn1,comp_bn2]
