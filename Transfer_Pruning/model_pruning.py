...
from Util.training_util import augment_all_images, LogTraining, DataSampling
...
from ResNet_hyperparams_pruning import *
...
def PrintExperimentStatus():
    ...
    print('Pruning Params: ' + '\n' + 
          ...
          '  MMD Number of Samples: ' + str(MMD_Scoring_Sample) + '\n' + 
          '  Balanced Sampling: ' +  str(is_balanced_sampling) + '\n' +
...
def main():
    ...
    for step in range(prune_step):
        ...
        # Pruning Process >>>
        # Prepare Data for Channel Scoring
        if Pfunc == 'MMD':
            Xpr, ypr = DataSampling(Xtr, ytr, num_sample=MMD_Scoring_Sample, balanced=is_balanced_sampling)
        else:
            Xpr, ypr = Xtr, ytr

        # Get the score for each channel in the network
        Net_Score_List = Get_Network_Channel_Score_List(
            layers = layers, X = Xpr, y = ypr, batch_size = 2000, images_pl = images_pl,
            mask_list = mask_list, mask_dict = mask_dict, sess = sess, func = Pfunc, info_print = True
        )
